...
Public Class TZBQServer
    ...
    Public Sub StartServer()
        Try
            log("TZBQ服务器已开启,端口:" & myPort)
            Dim th As New Thread(AddressOf ListenClient)
            th.Start()

        Catch ex As Exception
            log("TZBQ服务器开启失败，" & ex.Message)
            ...
            Try
                If Not _LocationListenSocket Is Nothing Then
                    ...
                    If Not clientSocket Is Nothing Then
                        log("TZBQ服务器>>新连接")
                        Dim TZBQDevice As New DeviceTZBQ(clientSocket)
                        AddHandler TZBQDevice.RefrushDeviceList, AddressOf sub_RefrushDeviceList
                        AddHandler TZBQDevice.RaiseLog, AddressOf sub_log
                        TZBQDevice.Start()
                    End If
                End If
            Catch ex As Exception
